...
import { createQueryWithToolResponsePrompt, createChatPrompt } from "../prompt_templates";
...
export class SequentialGenerator {
  ...
  async *createSequentialResponseGenerator(
    ...
    if (!modelSupportsTools) {
      TraceLogger.trace(requestContext, 'model-no-tools', `Model ${MODELS.CHAT_MODEL} does not support tools, using simple chat`);
    ...
    // ONLY if we support tools, do all the tool logic below
    ...
    try {
      ...
      while (!conversationComplete && !cancellationToken.isCancelled) {
        TraceLogger.trace(
          requestContext,
          "conversation-round",
          `Starting conversation round with ${conversationMessages.length} messages`
        );
        ...
        // Since we know the model supports tools
        ...
        for await (const part of response) {
          ...
          if (part.message.tool_calls && part.message.tool_calls.length > 0) {
            ...
            TraceLogger.trace(
              ...
              `Tool calls received in stream: ${part.message.tool_calls.map((tc) => tc.function.name).join(", ")}`
            );
            ...
            for (const toolCall of part.message.tool_calls) {
              ...
              const displayToolId = streamingTool
                ? streamingTool.toolId
                : `tool-${toolCallKey.replace(/[^a-zA-Z0-9-]/g, "-")}`;
              ...
              try {
                ...
                if (toolResponse.isError) {
                  resultText =
                    toolResponse.content?.map((c) => (c.type === "text" ? c.text : JSON.stringify(c))).join("") ||
                    "Tool call failed";
                } else if (!toolResponse.content || toolResponse.content.length === 0) {
                  resultText = "Tool executed successfully (no content returned)";
                } else {
                  resultText = toolResponse.content
                    .map((item) => (item.type === "text" ? (item.text as string) : JSON.stringify(item)))
                    .join("");
                }
        ...
        TraceLogger.trace(
          requestContext,
          "detected-tools",
          `Detected ${detectedToolCalls.length} tool calls: ${detectedToolCalls.map((tc) => tc.function.name).join(", ")}`
        );
