...
function UpdateNotification() {
  ...
  useEffect(() => {
    ...
    const handleUpdateStatus = (_event: any, data: any) => {
      ...
      // Use a ref to access the current userDismissed value
      const currentUserDismissed = userDismissedRef.current;
      const currentUpdateVersion = updateStatusRef.current.info?.version;
      
      if (currentUserDismissed && data.status === 'available' && 
          currentUpdateVersion === data.info?.version) {
        log.info('[UpdateNotification] Skipping notification - user dismissed this version');
        return;
      }
      
      // Reset user dismissed flag for new updates or important statuses
      if (data.status === 'available' && currentUpdateVersion !== data.info?.version) {
